...
export const TABLES = {
  ...
  // adminSettings replaces invoiceSettings — grouped JSON-blob columns (company/bank/vat/texts)
  // instead of ~20 flat attributes, see AdminSettingsRawRow. flattenAdminSettings() below unpacks
  // it back into the flat shape the rest of this function already expects.
  invoiceSettings: 'adminSettings',
};

/** Mirrors sites/project-manager/src/features/invoiceSettings/api.ts#flatten. */
export function flattenAdminSettings(row) {
  if (!row) return null;
  const parseGroup = (json) => {
    if (!json) return {};
    try {
