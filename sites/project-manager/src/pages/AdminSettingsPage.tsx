...
import { ResetToolsPanel } from '../features/admin/ResetToolsPanel';
import { IconCheck } from '../components/icons';
import { useInvoiceSettings, useUpdateInvoiceSettings } from '../features/invoiceSettings/hooks';
import type { PortalContext } from '../layouts/PortalLayout';

type SettingsTab = 'company' | 'invoicing' | 'tools';
type InvoicingSubTab = 'bank' | 'vat' | 'texts';
type ToolsSubTab = 'reset';
...
export function AdminSettingsPage() {
  ...
  const [invoicingTab, setInvoicingTab] = useState<InvoicingSubTab>('bank');
  const [toolsTab, setToolsTab] = useState<ToolsSubTab>('reset');
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      vatRateHigh: settings.vatRateHigh != null ? String(settings.vatRateHigh) : '21',
      vatRateLow: settings.vatRateLow != null ? String(settings.vatRateLow) : '9',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await updateSettings.mutateAsync({
        ...
        vatRateHigh: form.vatRateHigh.trim() ? parseFloat(form.vatRateHigh) : null,
        vatRateLow: form.vatRateLow.trim() ? parseFloat(form.vatRateLow) : null,
  ...
  return (
    <div className="content-card">
      <div className="content-inner admin-settings-page">
        <PageHeader
          title={<Trans>Instellingen</Trans>}
          description={<Trans>Algemene beheerinstellingen voor administrators.</Trans>}
          breadcrumb={
            <PageBreadcrumb
              items={[{ label: t`Dashboard`, to: '/app/dashboard' }, { label: t`Instellingen` }]}
            />
          }
          ...
          tabs={
            <div className="task-view-tabs" role="tablist">
              <button
                ...
                className={`task-view-tab${tab === 'company' ? ' active' : ''}`}
                onClick={() => setTab('company')}
              >
                <span><Trans>Company Details</Trans></span>
              </button>
              <button
                type="button"
                role="tab"
              ...
              >
                <span><Trans>Invoicing</Trans></span>
              </button>
              <button
                type="button"
                role="tab"
                aria-selected={tab === 'tools'}
                className={`task-view-tab${tab === 'tools' ? ' active' : ''}`}
                onClick={() => setTab('tools')}
              >
                <span><Trans>Tools</Trans></span>
              </button>
            </div>
          }
        />
        ...
        <div className="tab-panel-body">
          {isLoading && (tab === 'company' || tab === 'invoicing') ? (
            ...
            <form className="form invoice-settings-form" onSubmit={(e) => void handleSubmit(e)}>
              <div className="text-tab-bar" style={{ marginBottom: '1.25rem' }}>
                <button
                  type="button"
                  className={`text-tab ${invoicingTab === 'bank' ? 'active' : ''}`}
                  onClick={() => setInvoicingTab('bank')}
                >
                  <Trans>Bankdetails</Trans>
                </button>
                <button
                  type="button"
                  className={`text-tab ${invoicingTab === 'vat' ? 'active' : ''}`}
                  onClick={() => setInvoicingTab('vat')}
                >
                  <Trans>Vat & Payment Details</Trans>
                </button>
                <button
                  type="button"
                  className={`text-tab ${invoicingTab === 'texts' ? 'active' : ''}`}
                  onClick={() => setInvoicingTab('texts')}
                >
                  <Trans>Invoice texts</Trans>
                </button>
              </div>
                  ...
                  <fieldset>
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) => set('bankSwiftBic', e.target.value)}
                      />
                    </label>
                  </fieldset>
                )}
                ...
                {invoicingTab === 'vat' && (
                  <fieldset>
                    ...
                    <div className="form-row">
                      <label>
                        <Trans>BTW hoog tarief (%)</Trans>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          max="100"
                          value={form.vatRateHigh}
                          onChange={(e) => set('vatRateHigh', e.target.value)}
                          disabled={!form.vatEnabled}
                        />
                      </label>
                      <label>
                        <Trans>BTW laag tarief (%)</Trans>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          max="100"
                          value={form.vatRateLow}
                          onChange={(e) => set('vatRateLow', e.target.value)}
                          disabled={!form.vatEnabled}
                        />
                      </label>
                      <label>
            ...
            <div className="admin-tools-panel">
              <div className="task-view-tabs admin-tools-subtabs" role="tablist" aria-label={t`Tools`}>
                <button
                  type="button"
                  role="tab"
                  aria-selected={toolsTab === 'reset'}
                  className={`task-view-tab${toolsTab === 'reset' ? ' active' : ''}`}
                  onClick={() => setToolsTab('reset')}
                >
                  <span><Trans>Reset</Trans></span>
                </button>
              </div>

              {toolsTab === 'reset' && availableCompanies.length > 0 ? (
                <ResetToolsPanel companies={availableCompanies} />
              ) : (
                <p className="empty-state"><Trans>Geen bedrijf geselecteerd.</Trans></p>
              )}
            </div>
          )}
        </div>
